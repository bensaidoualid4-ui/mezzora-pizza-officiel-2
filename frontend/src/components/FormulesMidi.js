...
const FormulesMidi = () => (
  <div className="py-8 max-w-3xl mx-auto" data-testid="formules-midi-section">
    <div className="text-center mb-12">
      <p className="text-[var(--red)] text-xs tracking-[0.3em] uppercase mb-4">Du lundi au vendredi</p>
    ...
    <div className="space-y-6">
      {formulesMidi.map((f) => (
        <div key={f.id} data-testid={`formule-card-${f.id}`} className="flex justify-between items-start border-b border-[var(--cream)]/10 pb-6">
          <div>
            ...
            {f.exclusions && <p className="text-[var(--red)]/60 text-xs mt-1 italic">{f.exclusions}</p>}
          </div>
          <span className="text-[var(--red)] font-serif text-xl font-medium flex-shrink-0 ml-6">{f.price.toFixed(2)}€</span>
        </div>
      ))}
    </div>
  </div>
